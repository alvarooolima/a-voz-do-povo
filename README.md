...
Logo abaixo do topo, dois painéis claros lado a lado (no estilo "Melhores/Piores empresas" do Reclame Aqui), cada um com o título dentro: à esquerda **"O que você pode relatar"**, um carrossel de categorias; à direita **"Como funciona"**, com os 3 passos em cards e o atalho "Entenda o site em 1 minuto →". Os dois painéis têm a mesma altura. Em telas menores que 1000px eles se empilham.

O carrossel de categorias mostra **2 categorias por slide quando lado a lado** (3 slides), **3 por slide quando o painel ocupa a largura toda** (761–999px, 2 slides) e **1 por slide no celular** (até 760px). Cada card tem ícone, selo de posição, descrição e uma faixa com o número de relatos e resolvidos daquela categoria (calculado dos relatos reais), além de "Reportar" (leva ao formulário já com a categoria) e "Ver relatos →" (filtra o feed). Navega por setas, pontos, teclado (← →) ou deslizando (scroll-snap nativo, sem rotação automática). As categorias vêm do array `CATEGORIES` em `main.js`.
