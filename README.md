...
Logo abaixo do topo, dois painéis claros lado a lado (no estilo "Melhores/Piores empresas" do Reclame Aqui), cada um com o título dentro: à esquerda **"O que você pode relatar"**, um carrossel de categorias; à direita **"Como funciona"**, com os 3 passos em cards e o atalho "Entenda o site em 1 minuto →". Os dois painéis têm a mesma altura e se empilham abaixo de 1100px.

Os cards de categoria copiam as medidas dos cards do Reclame Aqui (≈213×248px, ícone de 42px, selo de posição de 34px, faixa de status de 54px), **3 por slide (2 slides)**, ou 1 por slide no celular (até 760px). Cada card é clicável ("Ver relatos" daquela categoria no feed) e mostra, na faixa de baixo, quantos relatos e quantos resolvidos a categoria tem (calculado dos relatos reais). O link "Reportar problema →" no rodapé do painel leva ao formulário. Navega por setas, pontos, teclado (← →) ou deslizando (scroll-snap nativo, sem rotação automática). As categorias vêm do array `CATEGORIES` em `main.js`.
