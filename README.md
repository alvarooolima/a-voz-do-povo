...
A seção "O que você pode relatar" é um carrossel no estilo do painel de "melhores empresas" do Reclame Aqui: **3 categorias por slide (2 slides)**, cada uma num card compacto com ícone, selo de posição (1º, 2º…), descrição e uma faixa de status com o número de relatos e resolvidos daquela categoria (calculado dos relatos reais). Em telas até 760px, onde 3 cards lado a lado ficariam apertados, mostra 1 categoria por slide. Navega por setas, pontos, teclado (← →) ou deslizando no celular (scroll-snap nativo, sem rotação automática). Cada card tem "Reportar" (leva ao formulário já com a categoria escolhida) e "Ver relatos →" (filtra o feed por aquela categoria). As categorias vêm do array `CATEGORIES` em `main.js`.
